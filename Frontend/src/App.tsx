...
import { api, artistName, getAlbums, getDashboard, getMusic, getPlaylists, getRecent, idOf, type Album, type Dashboard, type Music, type Playlist, type User } from '@/lib/api';
import { PlayerProvider, usePlayer } from '@/context/player';
import { LikedProvider, useLiked } from '@/context/liked';
...
function Root() { const session = useSession(); const [location] = useLocation(); return <PlayerProvider><LikedProvider enabled={!!session.user}><ErrorBoundary resetKey={location}><Routes user={session.user} save={session.save} /></ErrorBoundary></LikedProvider></PlayerProvider>; }
